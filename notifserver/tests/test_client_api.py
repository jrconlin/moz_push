...
from notifserver.clientagent import make_client_agent
from services.tests.support import TestEnv


# API Version to use when making requests to Client Agent
API = "1.0"
...
class ClientAgentTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.app = TestApp(make_client_agent(self.config))
    ...
    def create_queue(self):
        res = self.app.post("/%s/new_queue" % API,
    ...
    def create_subscription(self, token):
        res = self.app.post("/%s/new_subscription" % API,
            json.dumps({'token': token}),
            extra_environ={'REMOTE_USER': self.username})
        assert res.status_int == 200

    def remove_subscription(self, token):
        res = self.app.post("/%s/remove_subscription" % API,
            json.dumps({'token': token}),
            extra_environ={'REMOTE_USER': self.username})

    def send_broadcast(self, message):
        test_msg = "this is a broadcast message"
         
        self.set_credentials("testuser", "mypassword")
        res = self.app.post("/%s/send_broadcast" % API,
            test_msg, 
            extra_environ={'REMOTE_USER': self.username})
